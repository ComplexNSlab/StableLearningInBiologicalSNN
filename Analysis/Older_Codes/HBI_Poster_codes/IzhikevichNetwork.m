classdef IzhikevichNetwork < handle
   ...
   properties (Access = public)
       ...
       %% External Stimulation (kick input) Parameters 
           ...
           input_strength = 10
           input_pattern = [1:10; 5:5:50]' % first column is index of stimulated neurons and second is timing of stimulations
   ...
   methods (Access = public)
      ...
      function run(obj,T)
           ...
           for i=1:n_t % simulation of T in ms
                ...
                if obj.input
                    time_ = mod(round(obj.t), obj.input_interval);
                    if time_ <= max(obj.input_pattern(:, 2)) + obj.input_duration
                        I = I + obj.input_current(:, 1+ time_/obj.dt);
                    end
                end
   ...
   methods (Access = public)
      ...
      function SetInputCurrent(obj)

            obj.input_current = zeros(obj.N, round(obj.input_interval/obj.dt));
            indices = obj.input_pattern(:, 1);
            timings = obj.input_pattern(:, 2);
            
            for i = 1:size(obj.input_pattern, 1)
                obj.input_current(indices(i), timings(i)/obj.dt : (timings(i) + obj.input_duration)/obj.dt) = obj.input_strength;
            end
      end
