...
classdef IzhikevichNetwork < handle
   ...
   properties
       ...
       sampling_rate = 300 % sampling rate of slow variables (w)
      ...
      function run(obj,T)
           ...
           obj.dt = 0.1; % ms 
            ...
            for i=1:n_t % simulation of T in ms
                ...
                v_save(:, i) = obj.v;
                A_save(:, i) = obj.A;
                if mod(i, obj.sampling_rate) == 0 && obj.plasticity  
                     w_save(:, :, i/obj.sampling_rate) = obj.w;
                end
                ...
                if mod(i, 1000) == 0
                    waitbar(i/n_t,f, sprintf('please wait : %d %%', round(100*i/n_t)));
                end
            end
