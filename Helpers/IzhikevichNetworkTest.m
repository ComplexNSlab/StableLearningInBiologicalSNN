...
%% distance matrix of weights vector

w_ee = data.w(1:mynet.Ne, 1:mynet.Ne, :);
mask = mynet.Adjacency_matrix(1:mynet.Ne, 1:mynet.Ne);
w_ee = w_ee(repmat(mask, [1, 1, size(w_ee, 3)]));

w_ee = reshape(w_ee, sum(mask(:)), 400); % Reshape to (num_selected_elements × time)


mat = 1 - squareform(pdist(w_ee', "correlation"));
    
figure;
% imagesc(mat(201:end, 201:end))
imagesc(mat)
cb = colorbar;
colormap('parula');
set(gca, 'YDir', 'normal')
...
fig = figure('name', 'Weights Histogram', 'Position', [100, 100, 1920, 1080]);
for i = 1:5:size(data.w, 3)
    clf; % Clear the figure for the next histogram
    set(gca, 'fontsize', 30)
    ...
    histogram(data2(:, i), 100, 'Normalization', 'Pdf', 'EdgeColor', 'none', 'FaceColor', "#7E2F8E", 'FaceAlpha', 0.5, DisplayName="g_{ei}");
    histogram(data3(:, i), 100, 'Normalization', 'Pdf', 'EdgeColor', 'none', 'FaceColor', "#D95319", 'FaceAlpha', 0.5, DisplayName="g_{ie}");
    histogram(data1(:, i), 100, 'Normalization', 'pdf', 'EdgeColor', 'none', 'FaceColor', "#77AC30", 'FaceAlpha', 1, DisplayName="g_{ee}");
    legend()
    xlabel("w"); ylabel("PDF"); ylim([0, 10]);
...
% Shade the area between upper and lower bounds
% fillColor = [0.8, 0.8, 0.8]; % Light gray fill
fill(xPolygon, yPolygon1, 'k', 'EdgeColor', 'none', 'FaceAlpha', 0.5, 'HandleVisibility', 'off');

fill(xPolygon, yPolygon2, 'b', 'EdgeColor', 'none', 'FaceAlpha', 0.5, 'HandleVisibility', 'off');

if mynet.scaling    
    A_goal_Ex = mean(mynet.A_goal(1:mynet.Ne));
    A_goal_Inh = mean(mynet.A_goal(mynet.Ne+1:end));
...
for trial_number = 1:total_trials
    ...
    xlabel('time (ms)')
    ylabel('neuron index')
    title(sprintf('trial number %d raster plot, real time : %0.1f', trial_number, trial_number/10))
    plot([0, interval], (mynet.Ne + 0.5)*[1, 1], 'r-')
    plot([0, interval], (50 + 0.5)*[1, 1], 'b-')
    xlim([0, 40])
    ylim([0, mynet.N + 0.5])
